	...
	var rFilter = /^(image\/bmp|image\/gif|image\/jpg|image\/JPG|image\/jpeg|image\/png)$/i;
	var rFilter2 = /^(bmp|gif|jpg|JPG|jpeg|png)$/i;
	var nTotalSize = 0;
    ...
    function html5Upload() {
    	...
    	sUploadURL= '/api/attach/editor/upload'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
        ...
        jQuery.ajax({
            ...
            success: function (res) {
				...
				makeArrayFromString(res.resultText);
            },
			error: jindo.$Fn(onAjaxError, this).bind()
        });
    }
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.do',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.JPG;*.png;*.bmp;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)
